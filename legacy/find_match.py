...
def song_from_id(id):
    """
    returns corresponding song name from the song id
    """
    with open(DB_SONG_INFO, 'r') as f:
...
def recognize_and_match():
    """
    function to record audio clip and return the name of the song from the db
    """
...
def return_song(query_path):

    best_song_id, confidence = find_match(query_path)
    print(song_from_id(best_song_id))
    return (song_from_id(best_song_id),confidence)
